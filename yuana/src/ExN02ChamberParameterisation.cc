...
(const G4int copyNo, G4VPhysicalVolume* physVol) const
{
  G4double      Zposition= fStartZ;
	if(copyNo>23)
		Zposition = fStartZ-8*cm;
	else if(copyNo>11)
		Zposition = fStartZ+8*cm;
	else
		Zposition = fStartZ;;

  G4double 	Xposition = 10*sin(3.1415*copyNo/6)*cm;
  G4double 	Yposition = 10*sin(3.1415*(3-copyNo)/6)*cm;
  physVol->SetTranslation(G4ThreeVector(Xposition,Yposition,Zposition));
...
(G4Tubs& trackerChamber, const G4int copyNo, const G4VPhysicalVolume*) const
{
		if(copyNo>11)
		{
				trackerChamber.SetZHalfLength(0.001*cm);
				trackerChamber.SetInnerRadius(0);
				trackerChamber.SetOuterRadius(1*cm);
		}
		else
		{
			G4double  halfLength= (copyNo+1)*0.5*cm;
...
G4Material*  ExN02ChamberParameterisation::ComputeMaterial
